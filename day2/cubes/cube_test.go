...
func TestPowerOfGamesWithExample2(t *testing.T) {
	...
	cubes := GetCubesForAllGames(playedGames)

	power := map[int]int{}
	for gameId, gameCubes := range cubes {
		power[gameId] = GetPowerOfCubes(gameCubes)
	}

	if !reflect.DeepEqual(power, expected) {
		t.Errorf("Expected %v, but got %v", expected, power)
	}

}

func TestPowerOfGamesWithData2(t *testing.T) {

	playedGames, _ := util.ReadFile("testdata/part2.txt")
